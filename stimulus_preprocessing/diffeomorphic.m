...
maxdistortion= 30; % changes max amount of distortion
nsteps= 1; % number of steps
imsz= 1200; % size of output images (bigger or equal to 2 x input image)

picpath='Silhouette_objects';
outpicpath='Silhouette_diffeomorphic30';
...
for i=1:length(fns) %This is the number of objects in the directory
    ...
    for quadrant=1:4
        ...
        for j=1:nsteps %This is the number of steps - Total number of warps is nsteps * quadrant
            ...
            imwrite(uint8(interpIm), fullfile(outpicpath, fns(i).name), imgtype);
